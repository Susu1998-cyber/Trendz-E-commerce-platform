...
function SellerDashboard() {
  ...
  const fetchData = async () => {
    try {
      const result = await axios.get(
        `http://localhost:5000/api/admin/Product/viewProduct`
      );
  ...
  const deleteProduct = async (productId) => {
    try {
      await axios.delete(
        `http://localhost:5000/api/admin/Product/delete-product/${productId}`
      );
  ...
  return (
    ...
    <div className="seller-dashboard-container">
      <div className="text-end mt-4 me-4">
        <Link to={"/additem"}>
          <button className="btn btn-outline-dark me-5">Add Products</button>
        </Link>
        <button
